...
module.exports = (opts = {}) => {
    ...
    return async (tree, { cwd, path }) => {
        const possibleFrontMatter = tree.children[0];
        let proc_slug = ''
        if (possibleFrontMatter && possibleFrontMatter.type == 'yaml') {
            const data = yaml.safeLoad(possibleFrontMatter.value);
            path = path || data.__file__;
            proc_slug = data.proc_slug || '';
        }
        const assets = [];
        ...
        const handleUrl = async (url) => {
            ...
            return {
                ...
                slug: proc_slug,
                filename,
                url: join('media', proc_slug, filename),
            };
        };
